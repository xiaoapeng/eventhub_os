...
/* 判断哈希表节点是否需要重建 */
#define eh_hash_table_node_is_need_remake(head)  ((head)->next == NULL || (head)->prev == NULL)
...
static int eh_hashtbl_resize(struct eh_hashtbl *hashtbl){
    ...
    /* 将后半和table node 设置为NULL, 将在后面的插入中进行渐进式扩容 */
...
static void eh_hashtbl_node_remake(struct eh_hashtbl *hashtbl, unsigned int idx){
    struct eh_list_head *head = &hashtbl->table[idx];
    struct eh_list_head *pos, *n;
    unsigned int tmp_2_power;
    eh_list_for_each_safe(pos, n, head){
        struct eh_hashtbl_node *node = eh_list_entry(pos, struct eh_hashtbl_node, node);
        unsigned int new_idx = node->hash_val & hashtbl->mask;
        if(new_idx != idx){
            eh_list_del(pos);
            if(eh_hash_table_node_is_need_remake(hashtbl->table + new_idx)){
    ...
    for( tmp_2_power = (hashtbl->mask + 1U) >> 1; 
         (tmp_2_power & idx) == 0;
         tmp_2_power = tmp_2_power >> 1){
        unsigned int new_idx = idx | tmp_2_power;
        if(eh_hash_table_node_is_need_remake(hashtbl->table + new_idx)){
            eh_list_head_init(hashtbl->table + new_idx);
        }
    }
...
static void eh_hashtbl_try_remake(struct eh_hashtbl *hashtbl, unsigned int idx){
    /* 哈希渐进式重建 */
    if(eh_hash_table_node_is_need_remake(hashtbl->table + idx)){
        unsigned int mask_tmp = hashtbl->mask >> 1;
        for( mask_tmp = hashtbl->mask >> 1; 
             mask_tmp && eh_hash_table_node_is_need_remake(hashtbl->table + (idx & mask_tmp));
             mask_tmp = mask_tmp >> 1){
            /* find ... */
        }
        /* 用旧的 table node 进行重建 */
        eh_hashtbl_node_remake(hashtbl, (idx & mask_tmp));
    }
}
...
struct eh_hashtbl_node* eh_hashtbl_node_renew(eh_hashtbl_t hashtbl, 
    ...
    if(!eh_list_empty(&old_node->node)){
        eh_list_add(&old_node->node, &old_node->node);
...
int eh_hashtbl_insert(eh_hashtbl_t _hashtbl, struct eh_hashtbl_node *node){
    ...
    if(hashtbl->count + 1 >= hashtbl->threshold){
