...
__used void hardfault_handler_c(unsigned long sp, unsigned long lr , unsigned long control ){
    eh_task_sta_t                         sta;
    ...
    /* LR寄存器意义 Cortex-M7 Generic UG (r1p2, Issue C, 2018.11).pdf 章节： 2.3.7 Exception entry and return */
    eh_errln("LR:0x%08lx", lr);
    eh_errln("hardfault_handler  CONTROL:0x%08lx", control);


    /* 关于发生异常时，上下文堆栈情况  Cortex-M7 Generic UG (r1p2, Issue C, 2018.11).pdf 章节：Table 2-15 Exception return behavior  */
    eh_errln("dump sp:");
