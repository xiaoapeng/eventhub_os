...
__used void hardfault_handler_c(unsigned long sp, unsigned long lr , unsigned long control ){
    ...
    /* LR寄存器意义 Cortex-M3 Generic UG (r2p1, Issue A, 2010.12).pdf 章节：Table 2-17 Exception return behavior*/
    eh_errln("LR:0x%08lx", lr);
    eh_errln("hardfault_handler  CONTROL:0x%08lx", control);


    /* 关于发生异常时，上下文堆栈情况  Cortex-M3 Generic UG (r2p1, Issue A, 2010.12).pdf  章节：2.3.7 Exception entry and return  */
    eh_errln("dump sp:");
