...
/**
 * @brief 定义并初步初始化一个通用信号，在.c中使用，建议将自定义部分初始化后再进行注册
 */
...
#define EH_EXTERN_CUSTOM_SIGNAL(signal_name, event_type)                                \
    extern EH_STRUCT_CUSTOM_SIGNAL(event_type) signal_name;
...
/**
 * @brief 定义并初始化一个通用信号，在.c文件中定义后只需注册
 */
...
#define eh_signal_register(signal)                       \
    eh_event_cb_register((eh_event_t*)(&(signal)->event), &(signal)->trigger)

/**
 * @brief 信号注销，注销后无法再回调槽函数
 */
#define eh_signal_unregister(signal)                     \
    eh_event_cb_unregister((eh_event_t*)(&(signal)->event))
...
#define eh_signal_clean(signal)  do{                     \
        ...
        eh_event_clean((eh_event_t*)(&(signal)->event)); \
    }while(0)
