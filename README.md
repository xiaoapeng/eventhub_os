...
支持 Linux/macOS 主机平台，以及 Cortex-M/RISC-V MCU 架构。

| 测试芯片                    | 平台/架构          | 工具链                         | 备注                             |
|----------------------------|-------------------|--------------------------------|---------------------------------|
| i7-10700                   | linux/x86_64     | gcc 11.4.0                     |                                  |
| Apple Silicon              | macos/arm64      | clang                          | 对应 `CMAKE_SYSTEM_NAME=Darwin` |
...
| gd32vf103cbt6              | riscv32 (bumblebee n200) | riscv-none-embed-gcc 10.1.0 | 对应 `riscv32-bumblebee-n200`     |

### 本地构建与测试（当前仓库）

本仓库可以直接用 CMake 独立构建，不依赖上层 `fly` 工程。
...
调试构建（会启用 ASAN/LeakSanitizer + 覆盖率选项）：
...
顶层 CMake 会生成以下测试可执行文件（按源码实际）：

`test_co`、`test_eh`、`test_epoll`、`test_rb`、`test_coverage`、`test_safety`、`test_sleep`、`test_yield`、`test_mutex`、`test_sem`、`test_mem`、`test_dbg`、`test_ringbuf`、`test_signal`、`test_hashtbl`
...
#include <eh_event_flags.h>     /* 事件位图相关API */
...
| 状态成员 | 解释 |
| --- | --- |
| state | 任务状态:<br> `EH_TASK_STATE_READY`:就绪<br>`EH_TASK_STATE_RUNING`:运行<br>`EH_TASK_STATE_WAIT`:等待<br>`EH_TASK_STATE_FINISH`:结束<br>|
| stack | 任务栈起始地址 |
| stack_size | 任务栈大小 |
...
### 事件位图（event_flags）相关API

该模块用于按位等待/更新事件状态，接口定义在 `eh_event_flags.h`。

```c
extern __safety int eh_event_flags_init(eh_event_flags_t *ef);
extern __async int eh_event_flags_wait_bits_set(
    eh_event_flags_t *ef, eh_flags_t wait_flags, eh_flags_t claen_flags,
    eh_flags_t *reality_flags, eh_sclock_t timeout);
extern __async int eh_event_flags_wait_bits_clean(
    eh_event_flags_t *ef, eh_flags_t wait_flags, eh_flags_t set_flags,
    eh_flags_t *reality_flags, eh_sclock_t timeout);
...
void run(void){
    ...
    /* 反初始化是一个好习惯 */
    eh_signal_slot_disconnect(&test_signal, &test_signal_slot2);
    eh_signal_slot_disconnect(&test_signal, &test_signal_slot1);
...
void run(void){
    ...
    eh_signal_slot_disconnect(&timer_1000ms_signal, &test_signal_slot2);
    eh_signal_slot_disconnect(&timer_1000ms_signal, &test_signal_slot1);
    eh_timer_clean(eh_signal_to_custom_event(&timer_1000ms_signal));
}
